...
def generate_publish_data(csv_path: Path, output_path: Path) -> None:
    if not csv_path.exists():
        print(f"No publish CSV found at {csv_path}, skipping export.")
        return
    ...
    with csv_path.open("r", encoding="utf-8-sig") as fh:
        reader = csv.DictReader(fh)
        for row in reader:
            category = (row.get("題目類別") or "").replace("\n", " ").strip()
            if not category:
                continue

            entry_type = (row.get("展覽獎項論文") or "").strip()
            content = (row.get("內容") or "").replace("\r", "\n").strip()
            link = (row.get("連結") or "").strip() or None
            related = parse_related_codes(row.get("related_Work"))
    ...
    print(f"Generated {output_path} with {len(payload['groups'])} publish groups.")
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--skip-experience",
        action="store_true",
        help="Skip generating experienceData.json (use when experience is managed via admin backend).",
    )
    parser.add_argument(
        "--skip-publish",
        action="store_true",
        help="Skip generating publishData.json.",
    )
    ...
    # Experience data - skip if managed via admin backend
    if args.skip_experience:
        print("Skipping experienceData.json (--skip-experience flag is set)")
    else:
        experience_csv = Path("src/work_list/experience.csv")
        experience_output = Path("src/work_list/experienceData.json")
        generate_experience_data(experience_csv, experience_output)

    # Publish data
    if args.skip_publish:
        print("Skipping publishData.json (--skip-publish flag is set)")
    else:
        publish_csv = Path("src/work_list/publish.csv")
        publish_output = Path("src/work_list/publishData.json")
        generate_publish_data(publish_csv, publish_output)
