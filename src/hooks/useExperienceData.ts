...
/** Sentinel value larger than any real YYYYMM key so "present" sorts first. */
const PRESENT_KEY = 999999;

const sortExperiences = (items: ExperienceEntry[]): ExperienceEntry[] => {
  const withKeys = items.map((item) => {
    const parsedEnd = parseDateKey(item.end);
    // An empty / unparseable end date means "present" — sort it to the top.
    const endKey = parsedEnd > 0 ? parsedEnd : PRESENT_KEY;
    return {
      item,
      endKey,
